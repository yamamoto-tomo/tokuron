...
declare class EventTarget<EventMap extends Record<string, Event> = Record<string, Event>> {
     ...
     *
     * When set to true, options's passive indicates that the callback will not cancel the event by invoking preventDefault(). This is used to enable performance optimizations described in ﾂｧ 2.8 Observing event listeners.
     *
...
/**
 * A decoder for a specific method, that is a specific character encoding, like utf-8, iso-8859-2, koi8, cp1261, gbk, etc.ﾂ�A decoder takes a stream of bytes as input and emits a stream of code points. For a more scalable, non-native library, see StringView 窶� a C-like representation of strings based on typed arrays.
 *
...
/**
 * TextEncoder takes a stream of code points as input and emits a stream of bytes. For a more scalable, non-native library, see StringView 窶� a C-like representation of strings based on typed arrays.
 *
...
/**
 * This is the event type for fetchﾂ�events dispatched on theﾂ�service worker global scope. It contains information about the fetch, including theﾂ�request and how the receiver will treat the response. It provides the event.respondWith() method, which allows us to provide a response to this fetch.
 *
...
/**
 * This Fetch API interface allows you to perform various actions on HTTP request and response headers. These actions include retrieving, setting, adding to, and removing. A Headers object has an associated header list, which is initially empty and consistsﾂ�of zero or more name and value pairs. ﾂ�You can add to this using methods like append() (see Examples.)ﾂ�In all methods of this interface, header names are matched by case-insensitive byte sequence.
 *
...
/**
 * This Streams API interface represents a controller allowing control of aﾂ�WritableStream's state. When constructing a WritableStream, the underlying sink is given a corresponding WritableStreamDefaultController instance to manipulate.
 *
...
/**
 * This Streams API interface providesﾂ�a standard abstraction for writing streaming data to a destination, known as a sink. This object comes with built-in backpressure and queuing.
 *
...
/**
 * This Streams API interface providesﾂ�a built-in byte length queuing strategy that can be used when constructing streams.
 *
...
/**
 * The URLﾂ�interface represents an object providing static methods used for creating object URLs.
 *
...
interface BasicImageTransformations {
     ...
     * crop bottom or left and right sides as necessary, but won窶冲 crop anything
     ...
     * transparency (such as PNG). Accepts any CSS color (#RRGGBB, rgba(窶ｦ),
     * hsl(窶ｦ), etc.)
     */
    background?: string;
    /**
...
interface RequestInitCfProperties extends Record<string, unknown> {
     ...
     * declare a CNAME record within your own zone窶冱 DNS mapping to the
...
interface RequestInitCfPropertiesImage extends BasicImageTransformations {
     ...
     * to JPEG and WebP images. It doesn窶冲 have any effect on PNG.
     */
     ...
     *    (before and after resizing), source image窶冱 MIME type, file size, etc.
...
interface IncomingRequestCfPropertiesBotManagementBase {
    /**
     * Cloudflare窶冱 [level of certainty](https://developers.cloudflare.com/bots/concepts/bot-score/) that a request comes from a bot,
